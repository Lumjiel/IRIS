...
- **Human-in-the-loop**: on first reviewer FAIL (and `HITL_ON_REVIEW_FAIL`), `should_continue` routes to `hitl_gate` (asks user) → END instead of auto-replanning. A follow-up request with `hitl_choice` (retry/use_existing/redirect) routes to `apply_hitl` → replans (planner) or finalizes (END). Second FAIL auto-replans.
- **Anti-hallucination (grounded)**: writer only writes from retrieved content; empty content → honest fallback + `grounded=False`, never invokes LLM to invent.
- **Tracing**: optional Langfuse callback handler attached to graph config (`app/utils/tracing.py`); no-op if `LANGFUSE_*` keys absent.
- **Eval**: `backend/eval/run_eval.py` runs real cases through the graph and reports reviewer-pass / grounded / citations (needs real keys).
