  ...
  <aside class="w-64 shrink-0 h-full flex flex-col border-r border-slate-200/70 dark:border-slate-700/60 bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl">
    ...
    <div class="border-t border-slate-200 dark:border-slate-700 p-3 space-y-2">
      ...
      <div class="flex items-center gap-1">
        <router-link
          to="/market"
          class="flex-1 flex items-center justify-center gap-1 whitespace-nowrap px-1.5 py-1.5 rounded-md text-label text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors"
        >
...
const statusText = computed(() => {
  ...
  if (status.value.data_online) return `数据源：${status.value.data_source}`;
  return "数据源：内置模拟数据";
});
