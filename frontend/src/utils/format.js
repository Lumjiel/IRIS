...
export function parseChange(raw) {
  ...
  if (isNaN(num)) {
    return { display: String(raw), direction: 0, colorClass: 'text-slate-400', sign: '' };
  }
  const direction = num > 0 ? 1 : num < 0 ? -1 : 0;
  // A 股习惯：红涨绿跌
  const colorClass = direction > 0 ? 'text-up' : direction < 0 ? 'text-down' : 'text-slate-400';
  const sign = direction > 0 ? '+' : '';
  const display = direction === 0 ? `${num.toFixed(2)}%` : `${sign}${num.toFixed(2)}%`;
